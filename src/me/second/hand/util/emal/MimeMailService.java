package me.second.hand.util.emal;
...
import me.second.hand.entity.RegEmail;
import me.second.hand.entity.User;
import me.second.hand.util.EmailFile;
...
public class MimeMailService {
	 ...
	 * 发送MIME格式的用户修改通知邮件.
	 */
	public void sendNotificationMail(EmailSession emailSession,String emailFilePath,String title) {

//		String url="http://localhost:8080/superpanda/reg?registerId="+httpSession.getId();
		String url=emailSession.getRegId();
		try {
			MimeMessage msg = mailSender.createMimeMessage();
			MimeMessageHelper helper = new MimeMessageHelper(msg, true, DEFAULT_ENCODING);

			helper.setTo(emailSession.getEmail());
			helper.setFrom("in_side@sina.com");
			helper.setSubject(title);

//			StringBuffer stringBuffer=new StringBuffer();
//			stringBuffer.append("<html>");
//			stringBuffer.append("<body>");
//			stringBuffer.append("<h1>用户注册邮箱验证</h1>");
//			stringBuffer.append("<a href='http://www.baidu.com'>点击确认注册</a>");
//			stringBuffer.append("</body>");
//			stringBuffer.append("</html>");
			
			EmailFile emailFile=new EmailFile();
			
//			String content = generateContent(user.getUserName());
			
			String content =emailFile.getEmailHtmlByArge(emailFilePath,emailSession.getUserName(), url);
...
//			helper.addAttachment("mailAttachment.txt", attachment);
			...
			mailSender.send(msg);
			logger.info("HTML版邮件已发送至ycxxww@gmail.com");
